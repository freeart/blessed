...
Textarea.prototype.moveCursor = function(x, y){
  ...
  if (sync){
    ...
    var positionFromBegin = this.strWidth(prevText) + this.offsetX;
  ...
  if (key.name === 'escape') {
    ...
    if (!/^[\x00-\x08\x0b-\x0c\x0e-\x1f\x7f]$/.test(ch)) {
      if (this.offsetX === 0 && this.offsetY === 0){
        ...
        const copy = this._clines.slice();
        const cursorPosition = copy[currentLine].length + this.offsetX;
        copy[currentLine] = copy[currentLine].slice(0, cursorPosition) + ch + copy[currentLine].slice(cursorPosition);
        this.value = copy.join('\n');
      }
    }
  }
